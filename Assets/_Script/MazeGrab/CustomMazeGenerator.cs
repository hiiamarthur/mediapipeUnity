...
public class CustomMazeGenerator : MonoBehaviour
{
  ...
  [SerializeField] int amountOfMazes = 1;
  ...
  {
    Debug.Log("generateMazes");
    if (!Directory.Exists("Assets/Mazes"))
    {
      Directory.CreateDirectory("Assets/Mazes");

    }

    for (int i = 0; i < amountOfMazes; i++)
    {
      string name = "Maze" + (i + 1).ToString();
      GameObject newMaze = new GameObject(name);
      parent = newMaze;
    ...
    for (int i = 0; i <= Maze.GetUpperBound(0); i++)
    {
      for (int j = 0; j <= Maze.GetUpperBound(1); j++)
      {
        if (Maze[i, j] == 1)
        {
          ...
          ptype.transform.parent = parent.transform;
        }
        else if (Maze[i, j] == 0)
        {
